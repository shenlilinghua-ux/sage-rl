...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        scores = scores.unsqueeze(-1) * response_mask # 评估一下beam search和随机采样的准确率和长度
    # TODO add dif weight according to dif rollout source
    return scores, scores # [B, S]
...
def compute_policy_loss_vanilla(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor, # i.e. scores [B,S]
    response_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1) # [B,S] add ratio at every seq on this
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
...
# TODO 
@register_policy_loss("bpo")
def compute_policy_loss_bpo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor, # i.e. scores [B,S]
    response_mask: torch.Tensor,
    rollout_source: torch.Tensor, # [B,S]
    think_att_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    config: Optional[DictConfig | AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    assert config is not None
    assert not isinstance(config, AlgoConfig)
    clip_ratio = config.clip_ratio  # Clipping parameter ε for standard PPO. See https://arxiv.org/abs/1707.06347. # 0.2
    clip_ratio_low = config.clip_ratio_low if config.clip_ratio_low is not None else clip_ratio # 0.2
    clip_ratio_high = config.clip_ratio_high if config.clip_ratio_high is not None else clip_ratio # 0.2
    clip_ratio_c = config.get(  # Lower bound of the ratio for dual-clip PPO. See https://arxiv.org/pdf/1912.09729.
        "clip_ratio_c", 3.0
    )

    cliprange = clip_ratio # default 0.2
    cliprange_low = clip_ratio_low # default 0.2
    cliprange_high = clip_ratio_high # default 0.2
    ...
    # TODO apply element level clamp
    rollout_mask = rollout_source.expand_as(advantages)
    clip_low_tensor = torch.full_like(advantages, fill_value=1 - cliprange_low, dtype=torch.float32, device=advantages.device)
    clip_high_tensor = torch.full_like(advantages, fill_value=1 + cliprange_high, dtype=torch.float32, device=advantages.device)
    clip_high_tensor = torch.where(rollout_mask, config.beam_clip_ratio_high , clip_high_tensor)
    pg_losses2 = -advantages * torch.clamp(ratio, clip_low_tensor, clip_high_tensor)
    ...
    # TODO boost the think part of each rollout
    think_boost_ratio = config.think_boost_ratio
    pg_losses = torch.where(think_att_mask, pg_losses * think_boost_ratio, pg_losses)

    # TODO boost the ratio of beam search
    rollout_boost_ratio = config.rollout_boost_ratio
    pg_losses = torch.where(rollout_mask, pg_losses * rollout_boost_ratio, pg_losses)

    # print("boost the beam search rollout and the think part")
