...
class LLMEngine:
    ...
    def __init__(
        ...
        self.input_preprocessor = InputPreprocessor(self.model_config,
                                                    self.tokenizer,
                                                    mm_registry)
        # model_executor 真正执行forward的模型类，执行这行会开始加载模型
    ...
    def step(self) -> List[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        Details:
            - Step 1: Schedules the sequences to be executed in the next 调度
            ...
            - Step 2: Calls the distributed executor to execute the model. 执行
            - Step 3: Processes the model output. This mainly includes: 结果处理

                - Decodes the relevant outputs.
                - Updates the scheduled sequence groups with model outputs
                  based on its `sampling parameters` (`use_sage` or not).
        ...
        if not self._has_remaining_steps(seq_group_metadata_list):
            ...
            ctx.append_output(outputs=outputs,
                              ...
                              is_last_step=True,
                              is_first_step_output=is_first_step_output) # outputs 是一个列表，包含了
            # 一个prompt集合进行异步的解码
